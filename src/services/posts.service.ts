...
import { Post, PostData } from '../interfaces/post.interface';
...
export class PostsService {
  ...
  getPosts() {
    const postsResponse = this.posts.reduce((prev: PostData[], curr) => {
      const findAuthor = this.authors.find(
        (author) => curr.author_id === author.id
      );
      if (findAuthor) {
        prev.push({
          post: curr,
